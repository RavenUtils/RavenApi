...
public abstract class EssentialsMachineBlock extends EssentialsCommonMachineBlock {
  ...
  public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
    if (!worldIn.isClientSide) {
      ...
      if (tile instanceof EssentialsMachineTileEntity) {
        ...
        if (isItemToStartBurning(item) && !CommonTe.isBurning()) {
          ((EssentialsMachineTileEntity) tile).setIsBurning();
          item.hurtAndBreak(1, player, playerEntity -> playerEntity.broadcastBreakEvent(handIn));
  ...
  protected abstract boolean isItemToStartBurning(ItemStack item);
